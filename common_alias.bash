...
alias tsoff=$'tailscale status --json |jq \'.Peer[] | select(.Online|not) | .DNSName |= .[:-1] | .DNSName | select(contains(\"taila49c0.ts.net\")) + \" - Offline\"\''
alias tson=$'tailscale status --json |jq \'.Peer[] | select(.Online) | .DNSName |= .[:-1] | .DNSName | select(contains(\"taila49c0.ts.net\")) + \" - Online\"\''
